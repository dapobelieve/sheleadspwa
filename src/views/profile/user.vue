<template>
  <div class="d-flex flex-column profile">
    ...
    <div class="bio mt-12 mx-12" v-if="data.user">
      <div class="d-flex align-items-center">
        ...
        <span class="d-flex flex-column ml-12 text-grey-900">
          <span class="font-poppins text-bold" style="font-size: 16px">{{ data.user.first_name }} {{ data.user.last_name }}</span>
          <span class="text-grey-900 mt-8">{{ data.user.business_name }} | {{ data.user.industry }}</span>
        </span>
      </div>
      <div class="mt-8 d-flex flex-column mb-12">
        <span class="font-poppins mb-4">Leaderboard Position</span>
        <span>{{ data.leaderboard.position | ordinalSuffix }} | {{ data.leaderboard.points }}XP</span>
        <div class="mt-8" style="font-size: 12px">
          Business Bio
        </div>
        <span style="font-size: 12px" class="mt-8"> {{ data.user.business_location }}, Nigeria </span>
      </div>
    ...
    <div class="mt-12 ml-12">
      <div class="d-flex flex-column">
        ...
        <div>
          <div v-for="point in data.point" :key="point._id" class="d-flex align-items-center mt-12">
            <div class="text-white bg-primary mr-12 d-flex align-items-center justify-content-center" style="height: 44px; width: 44px; border-radius: 50%">
...
export default {
  ...
  methods: {
    ...mapActions(["getUserDetailsById", "logout"]),
    edit() {
      this.$router.push({
        name: "editProfile"
      });
  ...
  async mounted() {
    let res = await this.getUserDetailsById({
